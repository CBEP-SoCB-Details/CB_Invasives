...
## Duplicate Records
We use `pivot-wider()` followed by `pivot_longer()` and filter to identify 
duplicate records.
```{r}
the_data %>%
...
especially as relative abundances.
...
Note that this procedure retains any "real" NAs in the data, while it fills in 
values not present in the draft long data with "Absent".  This means we do not 
count data where species were recorded as present at all, which may not always 
be what we want.
```{r}
